...
export function createRoutes(args: {
  ...
  const warmupJobs = new Map<string, {
    id: string;
    status: "running" | "done" | "error";
    startedAt: string;
    finishedAt?: string;
    progress: { done: number; total: number; success: number; fail: number; skipped: number; lastCode?: string };
    result?: unknown;
    error?: string;
  }>();
  ...
  router.post("/api/pullback-chances", async (req, res) => {
    ...
    const codesInput: unknown[] = Array.isArray(req.body?.codes) ? (req.body.codes as unknown[]) : [];
    const codes: string[] = codesInput
  ...
  router.post("/api/pullback-cache/warmup", async (req, res) => {
    ...
    void warmupJQuantsBarsCache({
      ...
      cur.status = "error";
      cur.finishedAt = new Date().toISOString();
      cur.error = String((e as any)?.message ?? e);
    });
