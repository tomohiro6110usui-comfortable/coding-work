...
function resolveSelectionOptions(options) {
    ...
    const resolved = {
        minScore: Number.isFinite(options?.minScore) ? Number(options?.minScore) : minScore,
        maxPerBucket: Number.isFinite(options?.maxPerBucket) ? Math.max(1, Number(options?.maxPerBucket)) : Math.max(1, maxPerBucket),
        requireRebound: options?.requireRebound ?? requireRebound,
        minTurnover: Number.isFinite(options?.minTurnover) ? Math.max(0, Number(options?.minTurnover)) : Math.max(0, minTurnover),
        maxVolatility20: Number.isFinite(options?.maxVolatility20)
            ? Math.max(0.005, Number(options?.maxVolatility20))
            : Math.max(0.005, maxVolatility20),
        minPullbackPct: Number.isFinite(options?.minPullbackPct)
            ? clamp01(Number(options?.minPullbackPct))
            : clamp01(minPullbackPct),
        maxPullbackPct: Number.isFinite(options?.maxPullbackPct)
...
const dayBarsCache = new Map();
let nextRequestAt = 0;
let rateLimitedUntil = 0;
function barsDbKey(code, from, to) {
...
function computeDefaultFrom(toDate) {
...
async function getCachedRowsWithFallback(args) {
    ...
    const direct = await dbProvider.getJson(barsDbKey(code, from, to));
    ...
    for (let d = 1; d <= maxLookbackDays; d++) {
        ...
        const prev = await dbProvider.getJson(barsDbKey(code, prevFrom, prevTo));
...
async function getCachedRowsBatch(args) {
    ...
    if (!dbProvider || keys.length === 0)
        return {};
    if (typeof dbProvider.getJsonBatch === "function") {
        return (await dbProvider.getJsonBatch(keys)) ?? {};
    }
    const out = {};
    for (const key of keys) {
        const v = await dbProvider.getJson(key);
        if (v)
            out[key] = v;
    }
    return out;
...
function barsCacheKey(code, from, to) {
    return `${code}|${from}|${to}`;
}
function getCachedBars(code, from, to) {
...
function setCachedBars(code, from, to, rows) {
    const key = barsCacheKey(code, from, to);
    barsCache.set(key, { rows, expiresAt: Date.now() + BAR_CACHE_TTL_MS });
}
function getCachedDayBars(date) {
...
function setCachedDayBars(date, rows) {
    dayBarsCache.set(date, { rows, expiresAt: Date.now() + 60 * 60 * 1000 });
}
async function applyRequestPacing() {
    const now = Date.now();
...
function noteRateLimited(waitMs) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            ...
            if (res.ok) {
                ...
                let rows = (json?.data ?? json?.daily_quotes ?? []);
                if (rows.length === 0 && altCodes.length > 0) {
                    ...
                    if (altRes.ok) {
                        ...
                        rows = (altJson?.data ?? altJson?.daily_quotes ?? []);
                    }
                }
                if (rows.length > 0) {
...
async function fetchJQuantsDailyBarsByDate(args) {
    const apiKey = process.env.JQUANTS_API_KEY;
    if (!apiKey)
        throw new Error("Missing env: JQUANTS_API_KEY");
...
export async function checkJQuantsDailyBars(args) {
    ...
    const lastClose = last ? (last.AdjC ?? last.C) : null;
    return {
        code,
        from,
        to,
...
function evaluatePullback(args) {
    ...
    const reboundScore = 0.45 * clamp01((signals.dayGain + 0.01) / 0.03) +
        0.35 * clamp01((signals.rebound3 + 0.005) / 0.05) +
        0.2 * clamp01((signals.rebound5 + 0.01) / 0.08);
    const maScore = 0.65 * clamp01(((signals.sma5 / Math.max(signals.sma20, 1e-9)) - 0.98) / 0.06) +
    ...
    const liquidityScore = signals.avgTurnover20 === null
        ? 0.5
    ...
    if (signals.price >= signals.sma5)
        reasons.push("above SMA5");
    if (signals.sma5 >= signals.sma20)
        reasons.push("SMA5 >= SMA20");
    ...
    const grade = score >= 80 ? "A" : score >= 68 ? "B" : "C";
    const hasRebound = signals.dayGain >= 0 || signals.rebound3 >= 0.01 || signals.price >= signals.sma5;
    const passSelection = signals.dropFrom20High >= args.selection.minPullbackPct &&
...
export async function warmupJQuantsBarsCache(args) {
    ...
    if (codes.length >= 500) {
        ...
        const existingByKey = !force && args.dbProvider
    ...
    for (let i = 0; i < codes.length; i++) {
        ...
        try {
            if (!force && args.dbProvider) {
                const cachedRows = await args.dbProvider.getJson(barsDbKey(code, from, to));
...
export async function handlePullbackChances(args) {
    ...
    const failedOnInitial = new Set();
    const allowNetworkFetch = refresh || universeCodes.length <= 20;
    const processCode = async (code, phase, maxAttempts) => {
        if (dbProvider) {
        ...
        try {
            ...
            perCodeMap.set(code, {
                ...
                statusCodes: fetched.traces.map((t) => t.status).filter((x) => Number.isFinite(x)),
                phase,
            });
            ...
            perCodeMap.set(code, {
                ...
                error,
                traces,
                statusCodes: (traces ?? []).map((t) => t.status).filter((x) => Number.isFinite(x)),
                phase,
            });
            if (phase === "initial")
                failedOnInitial.add(code);
    ...
    const rankByScore = (a, b) => (b.score ?? 0) - (a.score ?? 0) || a.ratioNowHigh - b.ratioNowHigh || b.ratioHighLow - a.ratioHighLow;
    ...
    debug.nearMissTop = nearMisses
        .map((x) => {
        const shortGap = Math.max(0, scoringThresholds.shortMinRatioHighLow - x.shortRatioHighLow) +
            Math.max(0, x.shortRatioNowHigh - scoringThresholds.shortMaxRatioNowHigh);
        const midGap = Math.max(0, scoringThresholds.midMinRatioHighLow - x.midRatioHighLow) +
    ...
    debug.topScored = [...shortTerm, ...midTerm].sort(rankByScore).slice(0, 20);
    if (debug.failedCount > 0) {
        console.warn(`[pullback] date=${date} failed=${debug.failedCount} rateLimit429=${debug.rateLimit429Count} retried=${debug.retriedCount} recovered=${debug.recoveredByRetryCount}`);
    }
    const out = {
        date,
        key,
