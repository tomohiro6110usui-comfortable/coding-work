    ...
    router.get("/api/earnings-watchlist", async (req, res) => {
        ...
        try {
            ...
            if (isLegacySchemaError(e)) {
                ...
                res.json({ date, key: `${date}_earnings-watchlist`, items: data.items, fetchedAt: data.fetchedAt, source: "generated", warning: "sqlite schema mismatch; fallback=mock" });
                return;
            }
    ...
    router.get("/api/rankings", async (req, res) => {
        ...
        try {
            ...
            if (isLegacySchemaError(e)) {
                ...
                res.json({ date, key: `${date}_rankings`, items: data.items, fetchedAt: data.fetchedAt, source: "generated", warning: "sqlite schema mismatch; fallback=mock" });
                return;
            }
            res.status(500).json({ error: String(e?.message ?? e) });
        }
